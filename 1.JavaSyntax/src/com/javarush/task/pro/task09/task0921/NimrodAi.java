...
public class NimrodAi {
    ...
    public static void main(String[] args) {
        PastryLoader[] pastryLoaders = {new PastryLoader(), new PastryLoader(), new PastryLoader()};
            ...
            for (int i = 0; i < array.length; i++) {
                for (int j = 0; j < array[i].length; j++) {
                    for (int k = 0; k < array[i][j].length; k++) {
                       String pastry = array[i][j][k];
                        String string = pastry.toLowerCase();
                        if (string.contains("nimrod")){
                        array[i][j][k] = string.replaceAll("nimrod" , "pirate ship");
